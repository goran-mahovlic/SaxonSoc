...
# Configurations
SAXON_BUILDROOT_DEFCONFIG=saxon_ulx4m_defconfig
...
saxon_netlist(){
  cd $SAXON_SOC
  sbt "runMain saxon.board.radiona.ulx4m.Ulx4mSmp $SDRAM_SIZE"
}

saxon_bitstream(){
  cd $SAXON_SOC/hardware/synthesis/radiona/ulx4m/smp
  make clean
  make
}
...
saxon_ftp_load(){
  ...
  cp opensbi/build/platform/spinal/saxon/radiona/ulx4m/firmware/fw_jump.bin /var/ftp/pub/saxon/digilent/ArtyA7SmpLinux
...
saxon_fpga_load(){
  cd $SAXON_ROOT
  fujprog  SaxonSoc/hardware/synthesis/radiona/ulx4m/smp/bin/toplevel.bit
}
